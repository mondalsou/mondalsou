...
// ============================================
// WORK ITEMS CLOCKWISE ORBIT ON HOVER/TOUCH
// ============================================
(function () {
    const banner = document.querySelector('.current-work-banner');
    if (!banner) return;
    const ring = banner.querySelector('.work-orbit-ring');
    if (!ring) return;
    const items = Array.from(ring.querySelectorAll('.work-item'));
    if (items.length < 2) return;

    const RADIUS = 148;
    const SPEED  = 0.38; // degrees per frame — steady clockwise pace
    let angle    = 0;
    let raf      = null;
    let orbiting = false;
    ...
    function positionItems(deg) {
        ...
        items.forEach((item, i) => {
            const theta = (deg + i * step) * (Math.PI / 180);
    ...
    function startOrbit() {
        if (orbiting) return;
        orbiting = true;
        items.forEach(item => item.classList.remove('returning'));
    ...
    function stopOrbit() {
        ...
        raf = null;
        items.forEach(item => item.classList.add('returning'));
        ring.classList.remove('orbiting');
        items.forEach(item => { item.style.transform = ''; });
        setTimeout(() => items.forEach(item => item.classList.remove('returning')), 500);
    }

    banner.addEventListener('mouseenter', startOrbit);
    banner.addEventListener('mouseleave', stopOrbit);
    // Touch: tap to toggle orbit on/off
    banner.addEventListener('touchstart', () => orbiting ? stopOrbit() : startOrbit(), { passive: true });
})();
